...
description: Extend, tune, debug, or review the Rabbit Voxel Lab PlayCanvas template. Use for block registry changes, deterministic terrain or landmarks, chunk meshing, voxel collision and DDA, first-person controls, inventory and beacon rules, HUD/audio/effects, Rabbit lifecycle, performance work, or acceptance testing in this repository.
---
...
- Change geometry in `src/voxel/mesher.ts` and `src/entities/world-view.ts`. Keep one entity and one mesh per chunk, shared material, cross-chunk culling, neighbor invalidation, and the two-remesh-per-frame cap.
